...
namespace Sample
{
	public class DrawingTools
    {
        ...
        public static void DrawBarChart( Bitmap bitmap, Rectangle rect, Font font, decimal maxValue, BarData[] bars, string xLabel, string yLabel, string comment )
        {
             BarChart barChart = new BarChart( maxValue, bars, xLabel, yLabel, comment );
			 barChart.Draw( bitmap, rect, font );
        }
        ...
        {
            return DrawNamedValue( g, name, value, font, Brushes.Gray, point );
        }

        public static SizeF DrawNamedValue( Graphics g, string name, string value, Font font, Brush brush, PointF point )
        {
            string _name = name + ": ";
            SizeF size1 = g.MeasureString( _name, font );
            g.DrawString( _name, font, brush, point );
	...
	public class BarChart
	{
        ...
        public BarChart( decimal _maxValue, BarData[] bars, string xLabel, string yLabel, string _comment ) 
        {
            maxValue = _maxValue;
            if( maxValue == 0.0M ) {
				for( int index = 0; index < values.Length; index++ )
				{
					if( values[index].Value > maxValue )
					{
		...
		private void writeXSign( Graphics graphics )
		{
            ...
            SolidBrush commentBrush = new SolidBrush( Color.LightGray );
		...
		Decimal floorUp()
		{
			return Decimal.Floor( maxValue / findNearestPowerOfTen() );
		}
	}
}
